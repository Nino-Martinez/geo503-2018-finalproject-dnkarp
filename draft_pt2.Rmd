...
library(sf)
library(maps)
library(albersusa) : When trying to run this i got an error message (package ‘albersusa’ is not available (for R version 3.4.3) which version are you using? is there another package that I can use?
...
sfdf %>% 
  ...
  #Maybe filter the dataset by region (northeast, southwest, etc) and make individual plots so one can distinguish between states with greater ease
